...
test("Plex playback stays at the fixed 720p profile", () => {
  ...
  assert.equal(DEFAULT_PLEX_VIDEO_BITRATE_KBPS, 3000);
...
test("Plex fixed bitrate accepts a bounded override", () => {
  ...
  assert.equal(normalizePlexVideoBitrate("bad"), 3000);
  assert.equal(normalizePlexVideoBitrate(100), 3000);
  assert.equal(normalizePlexVideoBitrate(50_000), 3000);
});
